...
> = {
    ...
    'system.MinimumSoc': {
        ...
        signed: false,
        unit: '%',
        name: { en: 'Minimum SoC (except grid failure)', de: 'Minimum SoC (außer Netzausfall)' },
        write: true,
    },
    ...
    'system.DcFeedInEnabled': {
        ...
        signed: false,
        unit: '',
        name: { en: 'DC surplus to grid (overvoltage feed-in)', de: 'DC-Überschuss ins Netz (Overvoltage Feed-in)' },
        write: true,
...
class VictronGx extends utils.Adapter {
                ...
                if (remappedPath === 'Capacity') {
                    ...
                    if (!this.createdStates.has(literId)) {
                        await this.extendObjectAsync(literId, {
                            type: 'state',
                            common: {
                                name: { en: 'Capacity (liters)', de: 'Kapazität (Liter)' },
                                type: 'number',
                                role: 'value',
                                unit: 'l',
                                read: true,
    ...
    private getFriendlyName(path: string): ioBroker.StringOrTranslated {
        const names: Record<string, ioBroker.StringOrTranslated> = {
            ...
            'alarms.highVoltage': { en: 'Alarm: high voltage', de: 'Alarm: Überspannung' },
            ...
            FluidType: { en: 'Fluid type', de: 'Flüssigkeitstyp' },
