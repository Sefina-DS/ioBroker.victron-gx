...
> = {
    ...
    'system.DcFeedInEnabled': {
        ...
        signed: false,
        unit: '',
        name: 'DC-Überschuss ins Netz (Overvoltage Feed-in)',
        write: true,
...
class VictronGx extends utils.Adapter {
    ...
    private async testModbusWrite(): Promise<void> {
        ...
        for (let i = 0; i < 60; i++) {
            ...
            await new Promise<void>(resolve => { this.setTimeout(() => resolve(), 1000); });
        }
        if (!vebusEntry) {
    ...
    private async discoverModbusUnits(): Promise<void> {
        ...
        for (let unitId = 1; unitId <= 247; unitId++) {
            ...
            await new Promise<void>(resolve => { this.setTimeout(() => resolve(), 50); });
        }
